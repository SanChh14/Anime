...
def getrecent(site):
    ...
    recrel_data = [{'image':r.a.div['style'][17:-3], 'name':r.a.text.strip(), 'link':'/episode'+r.a['href'], 'episode':r.p.text} for r in recrel]
    return recrel_data
...
def home(request):
    ...
    ongoing_data = [{'link':a.a['href'], 'name':a.a.text.strip(), 'image': 'https://gogocdn.net/cover/'+a.a['href'][10:]+'.png', 'sub':a.a.text.strip()[-4:-1]} for a in ongoing[0:20]]

    #Recent Release
    r_release = site.find('div', class_='last_episodes loaddub').find_all('li')
    r_data = [{'name':r.p.a.text, 'image':r.div.a.img['src'], 'link':'/episode'+r.div.a['href'], 'episode':r.find_all('p')[1].text}  for r in r_release]
...
def category(request, slug):
    ...
    if status[1] != 'Upcoming':
        ...
        episodes = requests.get(url, headers = headers, verify=False).text
...
def episode(request, slug):
    ...
    url = 'https://gogoanime.pe/'+slug
    if(slug == 'hanasaku-iroha-episode-23'):
        return redirect('https://owoanime.com/')
    response = requests.get(url, headers = headers, verify=False).text
    site = BeautifulSoup(response, 'html.parser')
    ...
    videolink = 'https:'+site.find('iframe')['src']
